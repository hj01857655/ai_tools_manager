...
class WindsurfAutomation(BaseAutomation):
    """Windsurf自动化类"""
    ...
    def get_registration_url(self) -> str:
        return "https://codeium.com/windsurf/signup"
    
    def get_login_url(self) -> str:
        return "https://codeium.com/windsurf/login"
        ...
        try:
            ...
            if not self.wait_for_element('input[type="email"]', timeout=10):
                return AutomationResult(
                    status=AutomationStatus.FAILED,
                    message="注册页面加载失败",
                    screenshot_path=self.take_screenshot("windsurf_register_page_load_failed")
                )
            ...
            if not self.wait_and_input('input[type="email"]', data.email):
                return AutomationResult(
                    status=AutomationStatus.FAILED,
                    message="输入邮箱失败",
                    screenshot_path=self.take_screenshot("windsurf_register_email_failed")
                )
            ...
            if not self.wait_and_input(password_selector, data.password):
                return AutomationResult(
                    status=AutomationStatus.FAILED,
                    message="输入密码失败",
                    screenshot_path=self.take_screenshot("windsurf_register_password_failed")
                )
            ...
            if not register_clicked:
                return AutomationResult(
                    status=AutomationStatus.FAILED,
                    message="找不到注册按钮",
                    screenshot_path=self.take_screenshot("windsurf_register_button_not_found")
                )
            ...
            for indicator in verification_indicators:
                if indicator in page_text:
                    return AutomationResult(
                        ...
                        screenshot_path=self.take_screenshot("windsurf_register_email_verification"),
                        data={"email": data.email}
                    )
            
            # 检查是否注册成功
            if "dashboard" in current_url or "welcome" in current_url or "app" in current_url or "workspace" in current_url:
                return AutomationResult(
                    status=AutomationStatus.SUCCESS,
                    message="Windsurf账号注册成功",
                    screenshot_path=self.take_screenshot("windsurf_register_success"),
                    data=data.to_dict()
                )
                    ...
                    if error_element:
                        ...
                        if "already exists" in error_text.lower() or "已存在" in error_text:
                            return AutomationResult(
                                ...
                                screenshot_path=self.take_screenshot("windsurf_register_account_exists")
                            )
                        else:
                            return AutomationResult(
                                status=AutomationStatus.FAILED,
                                message=f"注册失败: {error_text}",
                                screenshot_path=self.take_screenshot("windsurf_register_error")
                            )
            
            return AutomationResult(
                status=AutomationStatus.UNKNOWN_ERROR,
                message="注册状态不明确，请手动检查",
                screenshot_path=self.take_screenshot("windsurf_register_unclear")
            )
        ...
        try:
            ...
            if not self.wait_for_element('input[type="email"]', timeout=10):
                return AutomationResult(
                    status=AutomationStatus.FAILED,
                    message="登录页面加载失败",
                    screenshot_path=self.take_screenshot("windsurf_login_page_load_failed")
                )
            ...
            if not self.wait_and_input('input[type="email"]', data.email):
                return AutomationResult(
                    status=AutomationStatus.FAILED,
                    message="输入邮箱失败",
                    screenshot_path=self.take_screenshot("windsurf_login_email_failed")
                )
            ...
            if not self.wait_and_input('input[type="password"]', data.password):
                return AutomationResult(
                    status=AutomationStatus.FAILED,
                    message="输入密码失败",
                    screenshot_path=self.take_screenshot("windsurf_login_password_failed")
                )
            ...
            if not login_clicked:
                return AutomationResult(
                    status=AutomationStatus.FAILED,
                    message="找不到登录按钮",
                    screenshot_path=self.take_screenshot("windsurf_login_button_not_found")
                )
            ...
            if "dashboard" in current_url or "app" in current_url or "workspace" in current_url:
                return AutomationResult(
                    status=AutomationStatus.SUCCESS,
                    message="Windsurf登录成功",
                    screenshot_path=self.take_screenshot("windsurf_login_success"),
                    data=data.to_dict()
                )
                    ...
                    if error_element:
                        ...
                        if "invalid" in error_text.lower() or "incorrect" in error_text.lower():
                            return AutomationResult(
                                ...
                                screenshot_path=self.take_screenshot("windsurf_login_invalid_credentials")
                            )
                        else:
                            return AutomationResult(
                                status=AutomationStatus.FAILED,
                                message=f"登录失败: {error_text}",
                                screenshot_path=self.take_screenshot("windsurf_login_error")
                            )
            ...
            for selector in captcha_selectors:
                if self.wait_for_element(selector, timeout=2):
                    return AutomationResult(
                        ...
                        screenshot_path=self.take_screenshot("windsurf_login_captcha_required")
                    )
            
            return AutomationResult(
                status=AutomationStatus.UNKNOWN_ERROR,
                message="登录状态不明确，请手动检查",
                screenshot_path=self.take_screenshot("windsurf_login_unclear")
            )
